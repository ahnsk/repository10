					...
					<td background="/images/ct_ttl_img02.gif" width="100%"
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							<tr>
								...
								<%} %>
								--%>
								<td width="93%" class="ct_tt101">상품 ${param.menu=='search' ? ' 목록조회':' 관리'}</td>
						...
						<table border="0" cellspacing="0" cellpadding="0">
							<tr>
								...
								<td background="/images/ct_btnbg02.gif" class="ct_btn01"
									...
									<!--  <a
									href="javascript:fncGetProductList();">검색</a>--> 검색
								</td>
...
<%}else if(menu.equals("search")) {%>
				...
				<c:forEach var="product" items="${list}">
					...
					<!-- 		<td align="left" value="prodName"><input type="hidden" value="${product.prodNo }"/>
								 <a href="/product/updateProduct?prodNo=${product.prodNo }">${product.prodName}</a>
								${product.prodName }</td>-->
		<!--  				<c:if test="${param.menu=='manage'}">
							<!--  부모의 아이디나 인풋 히든이나 이곳의 this를 찾아서 넣어줘야한다., ㄱ 
						</c:if>-->
<!--  				<c:if test="${param.menu =='search' }">
			...
			<% } %>
				 ...
				 --%> <input type="hidden" id="currentPage" name="currentPage"
						value="" /> <jsp:include
							page="../common/pageNavigatorProduct.jsp" />
